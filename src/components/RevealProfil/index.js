...
const RevealProfil = ({
  setCheckedProfiles, index, modeMission, profil, ...props
}) => {
        ...
        ? (
          <Grid container direction="column" justify="center">
            <Grid
              container
              direction="column"
              justify={small ? 'flex-start' : 'center'}
              alignItems={small ? 'flex-start' : 'center'}
              className={clsx(classes.firstBlock, { [classes.firstBlockModeMission]: modeMission })}
            >
              <Grid item container direction="row" justify="center" xs={12} md={12} className={classes.upCard}>
                ...
                <Grid item sm={12} md={8} direction="column" justify="center">
                  <Grid item className={classes.tagPreSelect}>
                    {checked && <Tag title="Profil pré-sélectionné" isPrimaryColor />}
                  </Grid>
                  <Typography
                    className={clsx(classes.name, { [classes.nameModeMission]: modeMission })}
                  >
                    {profil.firstName}
                    {' '}
                    {profil.lastName}
                      ...
                      >
                        ...
                        <Grid item>
                          <CustomSwitch
                            ...
                            onChange={() => setCheckedProfiles(index)}
                            switchSize="large"
                          />
                        </Grid>
                        <Grid item>
                          <Typography
                            variant="body2"
                            className={clsx(classes.preSelect, { [classes.selected]: checked })}
                          >
                            Profil
                            pré-sélectionné
                        </Typography>
              ...
              >
                <Grid xs={4} item container justify="center">
                  <CustomButton
                    xs={4}
                    title="Voir son CV"
                    handleClick={() => window.location.href = profil.linkedinLink}
                    className={clsx(classes.customButtonLeft, { [classes.customButtonModeMission]: modeMission })}
                  />
                </Grid>
                <Grid xs={4} item container justify="center">
                  <CustomButton
                    title="Voir son Portfolio"
                    handleClick={() => window.location.href = profil.portfolioLink}
            ...
            <Grid container direction="row" justify="space-between" spacing={1} xs={12}>
              ...
              <Grid item container xs={6} direction="column" justify="center" className={clsx(classes.blackCard, { [classes.blackCardModeMission]: modeMission })}>
                ...
                <Grid container direction="row" justify="center" className={classes.tagContainer}>
                  {
                    profil.expertises.map((item, index) => <Tag title={item.expertise.text} className={classes.tag} isPrimaryColor={item.priority} />)
                  }
