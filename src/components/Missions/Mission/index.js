...
export const Mission = ({ mission, ...props }) => {
  ...
  useEffect(() => {
    const getMissionStatus = (missionStatus) => {
      switch (missionStatus) {
        case 'WAITING_FOR_ACCEPTANCE':
          return 'Validation du brief en cours';
          break;
    ...
    const result = getMissionStatus(mission?.status);
              ...
              <Grid container direction={'row'} className={classes.container}>
                <Grid container className={clsx(classes.gridLeft, { [classes.gridLeftFinished]: props.status === 6 })}
                  ...
                  <Grid container item className={classes.statusContainer} direction={'row'}>
                    ...
                    <Typography
                      className={clsx(classes.statusTitle, { [classes.statusTitleRed]: props.status === 5 })}>{missionStatus.toUpperCase()}</Typography>
                  ...
                  <Grid item className={classes.titleContainer}>
                    <Typography className={classes.title}>{mission.brief.missionContext.title}</Typography>
                  </Grid>
