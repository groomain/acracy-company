...
export const Upload = (props) => {
  ...
  const [uploadedFiles, setUploadedFiles] = useState(props.name && companyData?.administrativeProfile?.legalDocuments && companyData?.administrativeProfile?.legalDocuments?.filter((file) => file.name === props.name).length !== 0
                  ...
                  <Box mx={2} key={`file-row${index}`}>
                    <Grid container direction="column" alignItems="center">
                      <div style={{ position: 'relative' }}>
                        <img src={fileIcon} alt="uploaded file" onClick={() => dispatch(getAttachmentsLaunched(companyData?.administrativeProfile?.legalDocuments?.filter((value) => value?.name === props.name)[0]?.externalId))} className={classes.img} />
                        <CloseIcon
                          hovered={hoveredClose}
                          onMouseEnter={() => setHoveredClose(true)}
                          onMouseLeave={() => setHoveredClose(false)}
                          className={classes.closeButton}
                          onClick={() => handleFileDeleteFromData(companyData?.administrativeProfile?.legalDocuments?.filter((value) => value?.name === props.name)[0]?.externalId)}
                        />
                      </div>
                      <Box my={1}>
